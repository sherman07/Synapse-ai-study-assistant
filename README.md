...
Password reset emails are generated by Supabase Auth but sent by Synapse through
the backend SMTP provider. Keep these values only in `backend/.env` and in the
Render `synapse-ai-backend` environment:
...
opens `frontend/reset-password.html` on the public frontend and the browser
uses the recovery session to update the password.
